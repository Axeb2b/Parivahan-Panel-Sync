...
async function proxyAadhar(
  ...
  if (ct.includes("application/json"))
    return { status: res.status, json: await res.json(), isJson: true };
...
router.post("/tool/aadhaar/initiate", requireAuth, async (req, res) => {
  try {
    ...
    const key = process.env.BADGER_API_KEY;
    if (!key)
      return res
        .status(503)
        .json({ error: "Aadhaar service is not configured" });
    const r = await proxyAadhar(
      "/api/initiate",
      { mobile, name: name || "MR" },
      key
    );
...
router.post("/tool/aadhaar/verify", requireAuth, async (req, res) => {
  try {
    ...
    if (!session_id || !otp)
      return res.status(400).json({ error: "session_id and otp required" });
    const key = process.env.BADGER_API_KEY;
    if (!key)
      return res
        .status(503)
...
router.post(
  "/tool/aadhaar/send-download-otp",
  requireAuth,
  async (req, res) => {
    try {
...
router.get("/tool/status", requireAuth, async (_req, res) => {
  return res.json({ ok: true, tool: "aadhaar" });
});
